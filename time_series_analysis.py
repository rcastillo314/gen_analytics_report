...
def plot_data(df, places, days, mode, column, filename, end_date):
    ...
    for index, place in enumerate(places):
        ...
        # Converts from total case count to daily case count
        counts = cumulative_data.iloc[:, start_column:].diff(axis=1)
        x_values = list(
            counts.columns[-days-offset:len(counts.columns)-offset])
...
def label_figure(x_values, y_values, mode, filename):
    ...
    # Helps ensure we don't add too many date tick marks
